...
const CompanyCard = require("./company.js");
const {
  commonBeforeAll,
  commonBeforeEach,
  commonAfterEach,
...
describe("create", function () {
  ...
  test("works", async function () {
    let company = await CompanyCard.create(newCompany);
  ...
  test("bad request with dupe", async function () {
    try {
      await CompanyCard.create(newCompany);
      await CompanyCard.create(newCompany);
...
describe("_filterWhereBuilder", function () {
  test("works with no filter criteria", function () {
    ...
    expect(CompanyCard._filterWhereBuilder(criteria)).toEqual({
      where: "",
      vals: [],
    });
  });
...
describe("findAll", function () {
  test("works: all", async function () {
    let companies = await CompanyCard.findAll();
  ...
  test("works: by min employees", async function () {
    let companies = await CompanyCard.findAll({ minEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("works: by max employees", async function () {
    let companies = await CompanyCard.findAll({ maxEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: by name", async function () {
    let companies = await CompanyCard.findAll({ nameLike: "1" });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: empty list on nothing found", async function () {
    let companies = await CompanyCard.findAll({ nameLike: "nope" });
    expect(companies).toEqual([]);
  });

  test("bad request if invalid min > max", async function () {
    try {
      await CompanyCard.findAll({ minEmployees: 10, maxEmployees: 1 });
      throw new Error("fail test, you shouldn't get here");
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
...
describe("get", function () {
  test("works", async function () {
    let company = await CompanyCard.get("c1");
...
describe("update", function () {
  ...
  test("works", async function () {
    let company = await CompanyCard.update("c1", updateData);
    expect(company).toEqual({
      handle: "c1",
      ...updateData,
    });
  ...
  test("works: null fields", async function () {
    ...
    let company = await CompanyCard.update("c1", updateDataSetNulls);
    expect(company).toEqual({
      handle: "c1",
      ...updateDataSetNulls,
    });
  ...
  test("not found if no such company", async function () {
    try {
      await CompanyCard.update("nope", updateData);
      throw new Error("fail test, you shouldn't get here");
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
